...
func (a *UserController) SignInHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		if email == "" {
			errMes := errorsmanager.HttpStatusFromCode("SIGNIN", errorsmanager.NO_EMAIL, "The email or nickname is required")
			errorsmanager.JsonError(w, errMes.Code, errMes)
			return
		}
		if password == "" {
			fmt.Println("--------------------------")
			errMes := errorsmanager.HttpStatusFromCode("SIGNIN", errorsmanager.NO_PASSWORD, "The password is required")
			errorsmanager.JsonError(w, errMes.Code, errMes)
			return
		}
